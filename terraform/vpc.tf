...
resource "aws_subnet" "public" {
  ...
  tags                    = merge(local.common_tags, { Name = "${var.project}-public-${local.azs[count.index]}" })
}
...
resource "aws_subnet" "private" {
  ...
  availability_zone = local.azs[count.index]
  tags              = merge(local.common_tags, { Name = "${var.project}-private-${local.azs[count.index]}" })
}
resource "aws_eip" "nat" {
  count  = local.nat_count
  domain = "vpc"
  tags   = merge(local.common_tags, { Name = "${var.project}-nat-eip-${count.index}" })
}
resource "aws_nat_gateway" "nat" {
  count         = local.nat_count
  allocation_id = aws_eip.nat[count.index].id
  subnet_id     = aws_subnet.public[count.index].id
  tags          = merge(local.common_tags, { Name = "${var.project}-nat-${count.index}" })
...
resource "aws_route_table" "private" {
  ...
  tags = merge(local.common_tags, { Name = "${var.project}-private-rt-${count.index}" })
}
